...
//        jmsTemplate.convertAndSend(logQueue, new ZvanjeLogDTO(null,jp.getSignature().toLongString(), "Pre izvršavanja metode. [LOGGED]. ", LocalDateTime.now(), "INFO"));
    ...
    @Around("execution(* rs.ac.singidunum.isa.app.controller.ZvanjeController.*(Long))")
    ...
    @After("execution(* rs.ac.singidunum.isa.app.controller.ZvanjeController.*(Long)) && args(id,..)")
